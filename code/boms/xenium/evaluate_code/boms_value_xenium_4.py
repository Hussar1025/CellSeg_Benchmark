...
"""eval_xenium_boms.py   v2026.08.14

Evaluate BOMS on the Xenium ROIs (xenium_4 liver / xenium_5 mouse brain /
xenium_6 colon).
...
the workbook, so the numbers drop straight into the 19-metric sheet.
  ...
  python eval_xenium_boms.py --dataset colon --qv-min 20
"""
...
DEFAULT_RUN = {"liver": "/data/qiuyijia/boms_xenium_4",
               "mouse_brain": "/data/qiuyijia/boms_xenium_5",
               "colon": "/data/qiuyijia/boms_xenium_6"}


def add_args(ap):
    ap.add_argument("--run-dir", default=None)
    ap.add_argument("--npz", default=None)
    ap.add_argument("--raster-grid", type=int, default=2000,
                    help="BOMS labels molecules, so a raster is built from the "
                         "per-RNA assignment for the pixel-level comparisons")
    ap.add_argument("--fill-raster-um", type=float, default=1.0,
                    help="grow that raster before measuring area, since a bare "
                         "scatter of molecules understates a cell badly")


def load_prediction(args, cfg, ps, S):
    """
    BOMS writes an npz, not a raster: cell_loc holds the centroids and seg holds
    one cell index per transcript.  Both are used -- the centroids directly, and
    a raster built from seg for coverage, area and the pixel-level comparison
    with the other methods.
    """
    d = Path(args.run_dir or DEFAULT_RUN[args.dataset])
    p = Path(args.npz) if args.npz else d / f"{d.name}.npz"
    if not p.exists():
        alt = find(d, "*.npz")
        if alt is None or "ckpt" in str(alt):
            raise SystemExit(f"no final npz under {d}; the run may still be "
                             "going (checkpoints live in ckpt/)")
        p = alt
    z = np.load(p, allow_pickle=True)
    log.info(f"npz         {p}   keys {list(z.files)}")
    cell_loc = np.asarray(z["cell_loc"], float)
    seg = np.asarray(z["seg"]).astype(np.int64)
    tx_x = np.asarray(z["x"], float)
    tx_y = np.asarray(z["y"], float)
    n = len(cell_loc)
    log.info(f"            {n:,} cells, {len(seg):,} assigned transcripts")
    for k in ("h_s", "h_r", "K", "epochs"):
        if k in z.files:
            log.info(f"            {k} = {z[k]}")
    ...
    x0u, y0u = cfg["x0"] * ps, cfg["y0"] * ps
    r = np.clip(((tx_y - y0u) / ps / scale).astype(np.int64), 0, g - 1)
    c = np.clip(((tx_x - x0u) / ps / scale).astype(np.int64), 0, g - 1)
    lab[r, c] = seg + 1
    occ0 = float((lab > 0).mean())
    if args.fill_raster_um > 0:
        from scipy.ndimage import distance_transform_edt
        rad = args.fill_raster_um / (scale * ps)
        dd, (iy, ix) = distance_transform_edt(lab == 0, return_indices=True)
        grow = (lab == 0) & (dd <= rad)
        lab[grow] = lab[iy[grow], ix[grow]]
    log.info(f"raster      {g}x{g} at {scale*ps:.3f} um/px: occupancy "
             f"{occ0*100:.2f}% -> {(lab>0).mean()*100:.2f}% after a "
             f"{args.fill_raster_um} um fill")

    ids, area, cx, cy, cov = raster_stats(lab, ps, scale)
    # the centroids come from BOMS itself, not from the raster
    keep = ids - 1
    pux = cell_loc[keep, 0]
    puy = cell_loc[keep, 1]
    note = ("no platform prior at inference, so the detection metrics are a real "
            "measurement; area comes from a filled raster of the per-RNA "
            "assignment, centroids come from BOMS cell_loc")
    if "gt_cells" in z.files:
        note += f"; run_meta gt_cells={int(z['gt_cells'])}"
