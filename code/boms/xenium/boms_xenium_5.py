#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""boms_xenium_5_run.py   v2026.08.14

BOMS on xenium_5 (Xenium 小鼠脑).

Self-contained.  Every dataset has its OWN ROI and they are not interchangeable,
so this file carries only one:

    ROI 中心 10000 native px  y0=6956 x0=12077  -> x[2566.4,4691.4] y[1478.2,3603.2] um  (2125 x 2125 um)
...
Patching, because run_boms indexes an n_tx x n_gene array and must stay under 2^31:
    ...
    25,858,179 tx x 5006 genes = 1.3e11  ->  9x9 = 81 patches
...
input.
    ...
    python boms_xenium_5_run.py --sweep-hs      # 扫 h_s，只跑一个 patch
    python boms_xenium_5_run.py --h-s <值>      # 正式跑，可断点续跑
    python boms_xenium_5_run.py --status        # 看进度
"""
...
def main():
    ...
    ap.add_argument("--dataset", default="xenium_5",
                    choices=["xenium_5"],
