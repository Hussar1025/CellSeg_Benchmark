#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""boms_xenium_4_run.py   v2026.08.14

BOMS on xenium_4 (Xenium 肝).

Self-contained.  Every dataset has its OWN ROI and they are not interchangeable,
so this file carries only one:

    ROI 中心 10000 native px  y0=13704 x0=21974  -> x[4669.5,6794.5] y[2912.1,5037.1] um  (2125 x 2125 um)
...
Patching, because run_boms indexes an n_tx x n_gene array and must stay under 2^31:
    ...
    1,430,729 tx x 377 genes = 5.4e8  ->  1 patch
...
input.
    ...
    python boms_xenium_4_run.py --sweep-hs      # 扫 h_s，只跑一个 patch
    python boms_xenium_4_run.py --h-s <值>      # 正式跑，可断点续跑
    python boms_xenium_4_run.py --status        # 看进度
"""
...
def main():
    ...
    ap.add_argument("--dataset", default="xenium_4",
                    choices=["xenium_4"],
