#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""boms_merfish_2_run.py   v2026.08.14

BOMS on merfish_2 (MERFISH 人肝癌 肝1).

Self-contained.  Every dataset has its OWN ROI and they are not interchangeable,
so this file carries only one:

    ROI mosaic 中心 10000 px  -> x[5236.7,6316.7] y[3811.0,4891.0] um  (1080 x 1080 um)
...
Patching, because run_boms indexes an n_tx x n_gene array and must stay under 2^31:
    ...
    2,387,644 tx x 550 genes = 1.3e9  ->  1 patch
...
input.
    ...
    python boms_merfish_2_run.py --sweep-hs      # 扫 h_s，只跑一个 patch
    python boms_merfish_2_run.py --h-s <值>      # 正式跑，可断点续跑
    python boms_merfish_2_run.py --status        # 看进度
"""
...
def main():
    ...
    ap.add_argument("--dataset", default="merfish_2",
                    choices=["merfish_2"],
