#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""boms_merfish_4_run.py   v2026.08.14

BOMS on merfish_4 (MERFISH 人肺癌).

Self-contained.  Every dataset has its OWN ROI and they are not interchangeable,
so this file carries only one:

    ROI mosaic 中心 10000 px  -> x[3783.0,4862.9] y[3346.8,4426.8] um  (1080 x 1080 um)
...
Patching, because run_boms indexes an n_tx x n_gene array and must stay under 2^31:
    ...
    3,642,281 tx x 550 genes = 2.0e9  ->  2x2 = 4 patches
...
input.
    ...
    python boms_merfish_4_run.py --sweep-hs      # 扫 h_s，只跑一个 patch
    python boms_merfish_4_run.py --h-s <值>      # 正式跑，可断点续跑
    python boms_merfish_4_run.py --status        # 看进度
"""
...
def main():
    ...
    ap.add_argument("--dataset", default="merfish_4",
                    choices=["merfish_4"],
