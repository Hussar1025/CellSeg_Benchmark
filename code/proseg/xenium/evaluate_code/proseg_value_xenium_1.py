...
EPS = 1e-12
SCRIPT_VERSION = "2026-05-25-proseg-transcript-eval-comparable-v1"
...
class EvaluationConfig:
    ...
    # GT is local on the server; benchmark files are pulled from GitHub into local_repo_dir.
    ...
    output_dir: str = "~/value_code/proseg_xenium1_sparse_hungarian_transcript_qc_results"
    method_name: str = "Proseg"
    ...
    pred_cell_id_col: str = "cell_id"

    # Transcript-level columns.
    # For Proseg, prediction transcript assignments are expected to use the same
    # predicted segmentation cell ID as the Proseg cell table, usually `cell_id`.
    # If your points/transcripts file uses a different column, pass
    # --pred-transcript-cell-id-col <column>.
    pred_transcript_cell_id_col: str = "cell_id"
...
def run_git_lfs_pull(repo_dir: Path) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            "Git LFS is required because the benchmark files in the GitHub repository are stored as LFS objects. "
...
def ensure_benchmark_repo(cfg: EvaluationConfig) -> Path:
    """Clone or update the CellSeg_Benchmark repository and return its local path.

    benchmark result files are stored in the GitHub repository. This function makes the
    ...
    if not repo_dir.exists():
        print(f"[INFO] Cloning CellSeg benchmark repo: {cfg.github_repo_url} -> {repo_dir}")
        subprocess.run(
        ...
        if cfg.github_pull:
            print(f"[INFO] Updating CellSeg benchmark repo in {repo_dir}")
    ...
    # The benchmark result files are stored with Git LFS. A normal git clone may only
...
def resolve_proseg_paths(cfg: EvaluationConfig) -> Tuple[Path, Path, Path]:
    """Resolve Proseg prediction files from explicit paths or from the cloned GitHub repo.

    Defaults are under:
      test/data/proseg_results/

    Cell-level prediction defaults to the AnnData/Zarr table at tables/table.
    If that directory is absent, it falls back to cell_polygons_proseg.geojson,
    where centroids are derived from polygons by read_geojson_cell_table().
    """
    repo_dir = ensure_benchmark_repo(cfg)
    default_dir = repo_dir / "test" / "data" / "proseg_results"

    default_cell_table = default_dir / "tables" / "table"
    fallback_geojson = default_dir / "cell_polygons_proseg.geojson"

    pred_cell_summary = (
        expand_path(cfg.pred_cell_summary)
        if cfg.pred_cell_summary
        else (default_cell_table if default_cell_table.exists() else fallback_geojson)
    )
    pred_objects = (
        expand_path(cfg.pred_objects)
        if cfg.pred_objects
        else default_dir / "proseg_instance_mask_fixed.tif"
    )
    pred_transcript_assignments = (
        expand_path(cfg.pred_transcript_assignments)
        if cfg.pred_transcript_assignments
        else default_dir / "points" / "transcripts"
    )
    ...
    if missing:
        raise FileNotFoundError(
            "Missing Proseg file(s) after cloning/pulling GitHub repository:\n" + "\n".join(missing)
        )

    pointer_files = [
        str(p) for p in [pred_cell_summary, pred_objects, pred_transcript_assignments]
        if p is not None and p.is_file() and is_git_lfs_pointer(p)
    ]
    if pointer_files:
        raise RuntimeError(
            "These Proseg files are still Git LFS pointer files, not real data:\n"
            + "\n".join(pointer_files)
...
def transcript_level_metrics(
    ...
    Prediction transcript assignments are joined through cfg.pred_transcript_cell_id_col, while GT transcripts are joined through cfg.gt_transcript_cell_id_col.
    """
    res: Dict[str, Any] = {}
    try:
    ...
    # are a matched pred-GT pair. This uses pred cfg.pred_transcript_cell_id_col and GT cfg.gt_transcript_cell_id_col.
...
def evaluate(cfg: EvaluationConfig) -> pd.DataFrame:
    ...
    pred_path, pred_objects_path, pred_transcript_assignments_path = resolve_proseg_paths(cfg)
...
def parse_args() -> EvaluationConfig:
    parser = argparse.ArgumentParser(description="Evaluate Proseg against Xenium_1 GT with sparse Hungarian matching + transcript-level QC.")
    ...
    parser.add_argument("--pred-cell-summary", default=None, help="Optional override. If omitted, use GitHub repo test/data/proseg_results/tables/table, falling back to cell_polygons_proseg.geojson")
    parser.add_argument("--pred-objects", default=None, help="Optional override. If omitted, use GitHub repo test/data/proseg_results/proseg_instance_mask_fixed.tif")
    parser.add_argument("--pred-transcript-assignments", default=None, help="Optional override. If omitted, use GitHub repo test/data/proseg_results/points/transcripts")
    parser.add_argument("--output-dir", default="~/value_code/proseg_xenium1_sparse_hungarian_transcript_qc_results")
    parser.add_argument("--method-name", default="Proseg")
    ...
    parser.add_argument("--pred-cell-id-col", default="cell_id", help="Cell ID column name in Proseg prediction cell table, e.g. cell_id.")
    parser.add_argument("--pred-transcript-cell-id-col", default="assignment", help="Prediction transcript assignment cell ID column. For Proseg this is usually cell_id; override if your points table uses another column.")
