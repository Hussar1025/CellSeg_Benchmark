...
"""eval_xenium_ucs.py   v2026.08.14

Evaluate UCS on the Xenium ROIs (xenium_4 liver / xenium_5 mouse brain /
xenium_6 colon).
...
the workbook, so the numbers drop straight into the 19-metric sheet.
  ...
  python eval_xenium_ucs.py --dataset colon --qv-min 20
"""
...
DEFAULT_RUN = {"liver": "/data/qiuyijia/ucs_xenium_liver_roi10000_platform_prior",
               "mouse_brain": "/data/qiuyijia/ucs_xenium_mouse_brain_roi10000_prior",
               "colon": "/data/qiuyijia/ucs_xenium_colon_bin5"}


def add_args(ap):
    ap.add_argument("--run-dir", default=None,
                    help="default: the per-dataset directory listed in the docstring")


def load_prediction(args, cfg, ps, S):
    import tifffile
    d = Path(args.run_dir or DEFAULT_RUN[args.dataset])
    p = Path(args.mask) if args.mask else d / "ucs_segmentation.tif"
    if not p.exists():
        alt = find(d, "ucs_segmentation.tif", "log*/pred/segmentation_mask.tif",
                   "*segmentation*.tif")
        if alt is None:
            raise SystemExit(f"no UCS raster under {d}; run --stage post")
        p = alt
    ...
    if mp.exists():
        ...
        note = (f"prior={m.get('prior')} bin_factor={m.get('bin_factor')} "
                f"nucleus={m.get('nucleus_diam_um')}")
        # the bin factor decides how many priors survive; UCS never adds cells,
        # so a ratio below 1 is a binning loss rather than a detection failure
        if m.get("prior_cells_on_grid") and m.get("platform_cells"):
            lost = m["platform_cells"] - m["prior_cells_on_grid"]
            if lost > 0:
                note += (f"; {lost:,} priors vanished at bin "
                         f"{m.get('bin_factor')}, so the cell count is capped "
                         f"at {m['prior_cells_on_grid']:,}")
