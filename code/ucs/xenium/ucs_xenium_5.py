...
def rasterise_fov_polygons(poly_path, fov, size, meta_fov):
    ...
    n = 0
    for i, (_, grp) in enumerate(df.groupby(cid, sort=True), 1):
        pts = np.rint(grp[[xc, yc]].to_numpy(float)).astype(np.int32)
        if len(pts) < 3:
            continue
        cv2.fillPoly(lab, [pts], int(i))
        n += 1
    logging.info(f"  rasterised {n:,} polygons; metadata lists "
                 f"{len(meta_fov):,} cells; coverage "
                 f"{(lab>0).mean()*100:.1f}%")
