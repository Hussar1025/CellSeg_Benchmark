...
Evaluate UCS on the MERFISH lung ROI.
...
def add_args(ap):
    ap.add_argument("--run-dir", default="/data/qiuyijia/ucs_merfish_lung_nuc")
...
def build(args, geo):
    ...
    if not p.exists():
        cand = sorted(d.glob("log*/pred/segmentation_mask.tif"))
        if not cand:
            raise SystemExit(f"no segmentation under {d}; run --stage post")
        p = cand[0]
    lab = np.squeeze(tifffile.imread(p)).astype(np.int64)
    log.info(f"mask        {p}   {lab.shape}")
    note = ""
    for name in ("prep_meta.json", "run_qc.json"):
        f = d / name
        if f.exists():
            m = json.loads(f.read_text())
            if "prior" in m:
                note = (f"prior={m['prior']}  reference_used="
                        f"{m.get('reference_used')}")
                log.info(f"{name}    {note}")
    s = args.roi_size / lab.shape[0]
    return Prediction(lab, s, "UCS", uses_platform_prior=True, note=note)
