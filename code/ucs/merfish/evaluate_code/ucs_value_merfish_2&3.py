...
UCS on the MERFISH liver ROIs.

UCS writes a label image on its own binned grid (1081 x 1081 here, i.e. one
grid pixel per micron).  That grid covers the shared 10000-mosaic-pixel ROI, so
the scale is simply 10000 / 1081 mosaic px per grid pixel and no extra
information is needed -- which matters, because the UCS output directories carry
no metadata file of their own.

  python eval_merfish_ucs.py --datasets liver1,liver2
...
def add_arguments(ap):
    ap.add_argument("--mask", default=None,
                    help="explicit path to the UCS segmentation mask")
    ap.add_argument("--px-per-cell", type=float, default=None,
                    help="mosaic px per grid pixel; default 10000/grid")


def build(dataset, cfg, args):
    root = Path(cfg["ucs_dir"])
    if args.mask:
        path = Path(args.mask)
    else:
        cands = [root / "ucs_log" / "pred" / "segmentation_mask.tif"]
        cands += sorted(root.glob("log*/pred/segmentation_mask.tif"))
        cands += sorted(root.glob("**/segmentation_mask.tif"))
        path = next((p for p in cands if p.exists()), None)
    if path is None or not path.exists():
        raise FileNotFoundError(f"no UCS segmentation mask under {root}")

    log.info(f"mask        {path}")
    lab = tifffile.imread(path)
    if lab.ndim != 2:
        lab = np.squeeze(lab)
    if lab.ndim != 2:
        raise RuntimeError(f"expected a 2-D label image, got {lab.shape}")
    if lab.shape[0] != lab.shape[1]:
        log.info(f"  the grid is not square ({lab.shape}); the scale is taken "
                 "from the first axis")

    s = args.px_per_cell if args.px_per_cell else ROI_SIZE / lab.shape[0]

    nuc = root / "nuclei_mask.tif"
    note = ""
    if nuc.exists():
        nm = tifffile.imread(nuc)
        n_prior = int(np.unique(nm[nm > 0]).size)
        ap = np.bincount(nm.reshape(-1).astype(np.int64))[1:]
        ap = ap[ap > 0]
        aa = np.bincount(lab.reshape(-1).astype(np.int64))[1:]
        aa = aa[aa > 0]
        exp = float(np.median(aa) / max(np.median(ap), 1))
        note = (f"nucleus prior {n_prior:,} cells, prior coverage "
                f"{(nm>0).mean()*100:.1f}%, area expansion {exp:.2f}x")
        if exp < 1.3:
            note += "  <- barely expanded, the prior is doing most of the work"

    return Prediction(labels=lab.astype(np.int64), px_per_cell=s, method="UCS",
